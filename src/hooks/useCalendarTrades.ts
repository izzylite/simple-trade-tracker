...
export function useCalendarTrades(options: UseCalendarTradesOptions) {
  ...
  const onTagUpdated = useCallback(async (
    ...
    // Batch update trades in parallel for better performance
    await Promise.all(
      tradesToUpdate.map(trade => {
        const updatedTags = trade.tags!.map(tag => tag === oldTag ? newTag : tag);
        return calendarService.getTradeRepository().update(trade.id, { tags: updatedTags });
      })
    );
    ...
    setCalendar({
      ...
      required_tag_groups: updateRequiredTagGroups(
        targetCalendar.required_tag_groups || [],
      ),
    });
     return { success: true, tradesUpdated: tradesToUpdate.length };
  }, [calendar, calendarId, trades]);
