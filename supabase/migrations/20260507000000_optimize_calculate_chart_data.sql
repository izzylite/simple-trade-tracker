-- Optimize calculate_chart_data RPC by materializing trades + economic_events once
--
-- Problem: previous version scanned public.trades ~14 times per call and
-- called jsonb_array_elements(economic_events) inside multiple EXISTS subqueries,
-- causing 4+ second warm execution and 8s statement timeouts on cold cache.
-- A single failure under React useEffect retries cascaded to PostgREST pool
-- exhaustion (522/524 origin errors).
--
-- Fix: collect filtered trades into pg_temp._t once, pre-flatten
-- economic_events into pg_temp._ev once, then run all aggregations against
-- the temp tables. Temp tables are per-transaction (ON COMMIT DROP); each
-- PostgREST request runs in its own transaction so isolation is preserved.
--
-- Output JSON shape is identical to migration 067. No client changes required.
...
BEGIN
  ...
  -- Materialize filtered trades ONCE for the lifetime of this transaction.
  -- pg_temp is implicitly searched even with empty search_path.
  ...
  -- Pre-flatten economic_events ONCE — replaces 6+ jsonb_array_elements scans.
  ...
  -- =====================================================
  -- 4. Economic correlations — driven by pre-flattened _ev
  -- =====================================================
  WITH
  -- Per-trade aggregates of which impact buckets each trade hits.
  ...
  -- Top-9 event names per impact, joined back to _t for trade arrays
...
runs all aggregations against the temp tables. Output JSON shape unchanged from migration 067.';
