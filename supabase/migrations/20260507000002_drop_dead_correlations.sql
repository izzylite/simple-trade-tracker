-- Phase B payload reduction: drop dead `losingTradeCorrelations` and
-- `winningTradeCorrelations` arrays from calculate_chart_data output.
--
-- Audit shows these fields were set into React state in
-- EconomicEventCorrelationAnalysis.tsx but never read again — pure dead
-- weight worth ~3MB on a 476-trade calendar. Counts that the UI does
-- consume live in `correlationStats.{losingTradesWithEvents,
-- winningTradesWithEvents}` and remain unchanged.
--
-- Schema delta vs migration 20260507000001:
--   high.losingTradeCorrelations    REMOVED
--   high.winningTradeCorrelations   REMOVED
--   medium.losingTradeCorrelations  REMOVED
--   medium.winningTradeCorrelations REMOVED
...
BEGIN
  ...
  SELECT COALESCE(account_balance, 0) INTO v_account_balance
  ...
  -- Economic correlations: only the stats counts + top-9 events are emitted.
  -- The big losingTradeCorrelations / winningTradeCorrelations arrays were
  -- removed in this migration (dead code on client).
  WITH
  ...
  high_impact_event_types AS (
    ...
    FROM (
      ...
      FROM (
        SELECT
          ev.event_name,
          jsonb_agg(t.id ORDER BY t.id) FILTER (WHERE t.trade_type = 'loss') as losing_trade_ids,
          jsonb_agg(t.id ORDER BY t.id) FILTER (WHERE t.trade_type = 'win')  as winning_trade_ids,
...
'Phase B slim: dropped dead losingTradeCorrelations + winningTradeCorrelations
arrays from output. Only correlationStats counts + mostCommonEventTypes (with
trade-id arrays) survive. Client must rehydrate IDs from top-level trades.';
