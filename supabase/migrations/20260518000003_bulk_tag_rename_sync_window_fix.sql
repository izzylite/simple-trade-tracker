-- Fix: bulk_update_tag_in_calendar's linked-calendar propagation was filtering by
-- the LINKED trade's created_at, but the original webhook path's isWithinSyncWindow()
-- checks the SOURCE trade's created_at. The two differ by ~ms in normal flow
-- (linked copies are auto-created right after source inserts via sync), so behavior
-- is nearly identical in practice — but using the source column preserves exact
-- parity with the pre-bulk-RPC behavior and avoids surprises if a linked copy is
-- ever created out-of-band.
--
-- This redefines the function body verbatim from 20260518000002 except for one CTE
-- column add and one WHERE-clause column change.
...
BEGIN
  ...
  IF v_linked_calendar_id IS NOT NULL THEN
    ...
    -- match isWithinSyncWindow() in handle-trade-changes/index.ts.
