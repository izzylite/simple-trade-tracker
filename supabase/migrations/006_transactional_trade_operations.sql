...
-- =====================================================
-- This migration creates PostgreSQL functions for atomic trade operations
-- that handle trade creation/update/delete AND calendar tag updates in a single transaction.

-- =====================================================
-- FUNCTION: add_trade_with_tags
-- =====================================================
-- Atomically creates a trade and updates calendar tags
...
-- =====================================================
-- FUNCTION: update_trade_with_tags
-- =====================================================
-- Atomically updates a trade and calendar tags
...
BEGIN
  ...
  -- Update the trade (only update fields that are provided)
  UPDATE trades
  SET
  ...
  IF p_trade_updates ? 'tags' AND jsonb_array_length((p_trade_updates->'tags')::JSONB) > 0 THEN
    -- Extract tags from trade
...
-- =====================================================
-- FUNCTION: delete_trade_transactional
-- =====================================================
-- Atomically deletes a trade (stats updated by trigger)
CREATE OR REPLACE FUNCTION delete_trade_transactional(
  p_trade_id UUID
)
RETURNS JSONB
LANGUAGE plpgsql
...
GRANT EXECUTE ON FUNCTION delete_trade_transactional(UUID) TO authenticated;
