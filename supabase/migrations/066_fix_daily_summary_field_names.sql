-- Fix daily summary field names to match component expectations
-- This migration fixes the field names in daily_summary to match DailySummaryTable.tsx:
-- 1. 'total_trades' -> 'trades'
-- 2. 'total_pnl' -> 'pnl'
-- 3. Add 'session' field (most common session for the day)
...
'Comprehensive chart data function with FIXED daily summary field names:
- trades (not total_trades)
- pnl (not total_pnl)
- session (most common session for the day using MODE() aggregate)
This matches the DailySummaryTable.tsx component expectations.';
