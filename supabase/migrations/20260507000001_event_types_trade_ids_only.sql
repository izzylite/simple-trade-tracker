-- Phase A payload reduction: replace full trade rows in mostCommonEventTypes
-- with arrays of trade IDs. Client rehydrates via Map<id, Trade> from the
-- top-level `trades` array. Eliminates ~1.2MB of duplicated trade JSON
-- (each trade was embedded once per matching event_name in top-9).
--
-- Schema delta vs migration 20260507000000:
--   high|medium → mostCommonEventTypes[].losingTrades   →  losing_trade_ids:  UUID[]
--   high|medium → mostCommonEventTypes[].winningTrades  →  winning_trade_ids: UUID[]
--
...
BEGIN
  ...
  -- Top-9 event names per impact. Trade lists emitted as ID arrays; client
  -- rehydrates from top-level `trades` payload.
...
'Optimized + slimmed: mostCommonEventTypes emits losing_trade_ids/winning_trade_ids
arrays instead of full trade rows. Client must rehydrate from top-level `trades`.';
