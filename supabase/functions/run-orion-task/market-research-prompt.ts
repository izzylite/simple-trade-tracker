...
TOOL USE — scrape_url (use when snippets are inconclusive):
You have access to a scrape_url tool that fetches the full body text of a news article. Snippets often hint at a catalyst without the trader's-eye details: direction (dovish vs hawkish), magnitude (50bps vs 25bps), source (head of state vs aide), or whether a "policy shift" is a real shift or just speculative analysis. Scrape any item where the snippet is too thin to judge those things AND the topic is relevant to the trader's markets (central banks, political statements, geopolitics, scheduled data, commodities, bond moves). Skip items already covered in the Previously Reported section — they add no new signal regardless of depth. Skip pure session color and routine market-roundup posts — the snippet already tells you everything there is. You may call scrape_url in parallel for up to 5 URLs in a single turn. URLs must come from the news lists in the user prompt.
