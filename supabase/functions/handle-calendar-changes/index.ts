...
interface CalendarWebhookPayload {
  ...
  old_record: Calendar | null
  new_record: Calendar | null
  calendar_id: string
  user_id: string
}
...
async function cleanupCalendarImages(
  ...
  try {
    ...
    const deleteChecks = await Promise.all(
      Array.from(imageIdsToCheck).map(async (imageId) => {
        const canDelete = await canDeleteImage(supabase, imageId, calendarId)
        return { imageId, canDelete }
      })
    )
    ...
    const { successCount, totalCount } = await deleteTradeImages(
      supabase,
      Array.from(imageIdsToDelete),
      userId,
      log
    )
 ...
 */
async function handleDelete(calendarId: string, userId: string, oldRecord: Calendar | null) {
  ...
  const cleanupResults = await Promise.allSettled([
    ...
    cleanupCalendarImages(calendarId, userId),
 ...
 */
async function handleInsert(calendarId: string, userId: string, newRecord: Calendar | null) {
 ...
 */
async function handleUpdate(calendarId: string, userId: string, oldRecord: Calendar | null, newRecord: Calendar | null) {
  log(`Processing UPDATE operation for calendar ${calendarId}`)

  // Check if calendar is marked for deletion
  if (newRecord?.mark_for_deletion && newRecord?.deletion_date) {
    // Check if deletion_date changed (cron updated it)
    ...
    if (deletionDateChanged) {
      ...
      // After successful cleanup, delete the calendar
      try {
        ...
        // Cleanup was successful but deletion failed
        // Calendar will be retried on next cron run
  ...
  // TODO: Add logic for other calendar updates if needed
  // For example: handle calendar name changes, year changes, etc.
