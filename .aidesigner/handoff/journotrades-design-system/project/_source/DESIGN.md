...
### Primary
- **Trader Violet** (#7c3aed): the only saturated brand color. Used on the active side-nav tile (solid pill), primary buttons, focus rings, key indicators in charts. Cap visible coverage on any single screen.
...
- **Title** (600, 1.125rem, line-height 1.4): card subheaders, dialog titles, the calendar selector trigger label.
...
### Named Rules
**The No-Glass Rule.** No `backdrop-filter: blur` decoration on cards, navs, or panels. The single permitted use is the calendar-lock-overlay backdrop and the calendar-switching spinner overlay, both of which obscure live content for a reason.
...
The vertical icon-rail (`SIDE_NAV_WIDTH = 92px`) is composed of three tiers separated by hairline dividers: a top **Create** action, the primary **NAV_ITEMS** stack, and a demoted **utility** tier (currently just About).
...
- **Default tile:** transparent background, icon in `text.secondary` (Slate 400 / Slate 500). Hover fills the tile with `theme.palette.action.hover` (~4% alpha tint on dark, ~4% on light) and lifts the icon to `text.primary` / Slate 100.
...
- **Hairline dividers:** 1 px `theme.palette.divider` strips above the route stack and above the utility tier. They mark the demotion in hierarchy without adding chrome.
- **Below `lg`** (the breakpoint, not the spacing token), the rail collapses into a temporary drawer at the same width.
...
- **Trigger:** a flat ButtonBase composed of a 32 px rounded avatar (calendar hero image or a violet-tinted CalendarIcon at 18% alpha), the calendar name in Title-size weight 600, and an 18 px chevron that rotates 180° when the menu is open. Hover applies `action.hover`. Focus ring is a 3 px violet at **25% alpha** glow around the trigger — slightly stronger than the input focus glow (15%) so the bar's focus state survives next to the page's own busy header chrome.
...
- **Backdrop:** 85% alpha `background.default` over the parent surface, with a 4 px `backdrop-filter: blur` — one of the only two permitted uses of blur in the system.
